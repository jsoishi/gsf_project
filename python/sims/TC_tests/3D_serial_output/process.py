    ...
    with h5py.File(filename, mode='r') as file:
        for index in range(start, start+count):
            ...
            for k, task in enumerate(file['tasks']):
                ...
                pcolormesh(mfig, k, task, dset, index, axis=3,x_index=16)
            # Title
...
def pcolormesh(mfig, k, taskname, dset, t_index, axis=0, x_index=0):

    # Pick data axes for x and y plot axes
    # Note: we use the full time-space data array, so remember that axis 0 is time

    # datslices --> Slices for data.
    # First is time axis, here sliced by the "t_index" argument.
    # The "xi" and "yi" entries should be "slice(None)",
    # Others (for >2 spatial dimensions) given by x_index.

    if axis == 1:
        xi, yi = (2, 3)
        datslices = (t_index, x_index, slice(None), slice(None))
    elif axis == 2:
        xi, yi = (1, 3)
        datslices = (t_index, slice(None), x_index, slice(None))
    else:
        xi, yi = (1, 2)
        datslices = (t_index, slice(None), slice(None),x_index)
